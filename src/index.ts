...
import { httpServer, checkSchedule, checkClockFontSize, checkMessageFontSize, setMainWindowForServer } from './server';
...
const createWindow = (): void => {
  ...
  mainWindow = new BrowserWindow({
    ...
    icon: path.join(__dirname,'/pause-indicator.ico'),
...
function initFontSize(){
  setTimeout(() => {
  const currentClockFontSize = checkClockFontSize()
  if(currentClockFontSize !== undefined){
    if(currentClockFontSize.data !== undefined){
      if(currentClockFontSize.data.fontSize !== undefined){
        mainWindow.webContents.send("clock-font-size-change", currentClockFontSize.data.fontSize);
      }
    }
  }

  const currentMessageFontSize = checkMessageFontSize()
  if(currentMessageFontSize !== undefined){
    if(currentMessageFontSize.data !== undefined){
      if(currentMessageFontSize.data.fontSize !== undefined){
        mainWindow.webContents.send("message-font-size-change", currentMessageFontSize.data.fontSize);
      }
    }
  }
  }, 1000*10);
...
function sendScheduleToFrontend(scheduleData:any,date:Date) {
  ...
  if(scheduleData.clockColor !== undefined && scheduleData.counterColor !== undefined && scheduleData.messageColor !== undefined){
    mainWindow.webContents.send("font-color-change", {clockColor: scheduleData.clockColor, counterColor: scheduleData.counterColor, messageColor: scheduleData.messageColor});
  }
}
